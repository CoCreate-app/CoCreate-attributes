...
function initElements(inputs, el) {
	for (let input of inputs) initElement(input, el);
}
...
async function initElement(input, el) {
	try {
		...
		if (selector.trim().endsWith(";")) {
			...
			if (input.hasAttribute("actions")) return;
			let { element, type, property, camelProperty } = await parseInput(
				input,
				el
			);
			if (!element) return;
...
function attributeTrigger({
	...
	const resizeObserver = new ResizeObserver((e) => {
		...
		for (let [
			input,
			{ element, type, property, camelProperty, isColl, from, to }
		] of triggers) {
...
function addClickEvent(input, selector) {
	...
	if (input.classList.contains("color-picker")) getPickr(container);
	else {
...
async function elClicked(e) {
	...
	for (let [input] of inputs) {
		...
		let attribute;
		if (input.id) attribute = input.id;
		else attribute = input.getAttribute("attribute-property");
		if (!attribute) attribute = input.getAttribute("attribute");
...
async function parseInput(input, element) {
	if (!element) {
		...
		} else element = initDocument.querySelector(selector);
	...
	let camelProperty,
		property = input.getAttribute("attribute-property");
	if (property) {
...
async function updateElement({
	...
	let inputValue =
		collValue != undefined ? collValue : await getInputValue(input);
	if (!inputValue) return;

	if (!Array.isArray(inputValue)) {
		inputValue = unit && inputValue ? inputValue + unit : inputValue;
		inputValue = removeZeros(inputValue);
	} else inputValue.forEach((a) => removeZeros(a.value));

	let hasUpdated = updateElementValue({
		...rest,
		type,
		property,
		camelProperty,
...
function updateElementValue({
	...
	switch (type) {
		case "classstyle":
			...
			// }
			value =
				inputValue && !hasCollValue ? inputValue + unit : inputValue;
			value = value || "";
		...
		case "style":
			...
			value = value || "";
			computedStyles = getRealStaticCompStyle(element);
			return setStyleIfDif.call(element, {
				property,
				camelProperty,
				value,
				computedStyles
...
function updateInput({ type, property, camelProperty, element, input }) {
	...
	switch (type) {
		...
		case "classstyle":
			...
			if (ccStyle[camelProperty]) value2 = ccStyle[camelProperty];
			else {
				computedStyles = getRealStaticCompStyle(element);
			...
			if (!value2) {
				return console.warn(
					`"${property}" can not be found in style object`
				);
			}
